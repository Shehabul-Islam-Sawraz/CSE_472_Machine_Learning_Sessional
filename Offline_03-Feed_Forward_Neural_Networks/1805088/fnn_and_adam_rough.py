...
def getTrainAndValidationData():
    ...
    # y_train = np.eye(27)[y_train]
    # y_test = np.eye(27)[y_test]
...
def getReportData(model, rate, epochs, batch_size, modelName):
...
def getReport():
    ...
    for rate in learning_rates:
        for i in range(len(layers)):
            ...
            modelName = ''.join([char for char in modelName if not char.islower()])
            ...
            print(modelName)
            getReportData(model, rate, epochs, batch_size, modelName)
            sys.stdout.close()
...
def test():
    model = NeuralNetwork([DenseLayer(28*28, 128), Sigmoid(), DenseLayer(128, 26), SoftmaxLayer()], 0.0005)
    getReportData(model, 0.0005, 4, 64, "TestingModel")
    model.dumpPickleFile()
